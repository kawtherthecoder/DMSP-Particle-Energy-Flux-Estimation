...
class SSJDay(object):
    ...
    def read_cdf_ssj(self):
        ...
        with pycdf.CDF(ssj_file) as cdffile:    
            ...
            # ADDED BY KAWTHER: READING ELE AND ION COUNTS FOR EACH CHANNEL
            self['ion_counts_bkg'] = cdffile['ION_COUNTS_BKG'][:]
            self['ion_counts_obs'] = cdffile['ION_COUNTS_OBS'][:]
            self['ele_counts_bkg'] = cdffile['ELE_COUNTS_BKG'][:]
            self['ele_counts_obs'] = cdffile['ELE_COUNTS_OBS'][:]
